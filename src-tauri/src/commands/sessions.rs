...
pub fn get_session(id: String) -> Result<SessionMessagesResponse, String> {
    ...
    let messages: Vec<SessionMessage> = message_rows
        ...
        .filter_map(|row| {
            ...
            // Only filter very specific Hermes tool output patterns that are single-line JSON objects
            let raw_content_check = raw_content.trim();

            if raw_content_check.starts_with('{')
                && raw_content_check.ends_with('}')
                && raw_content_check.len() < 500  // Only short JSON objects are tool output
...
fn cleanup_common_patterns(content: &str) -> String {
    ...
    result = remove_lines_containing(&result, "架构-");

    result = remove_lines_containing(&result, "继续检查状-");
