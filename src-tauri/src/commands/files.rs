...
pub async fn list_directory(
    ...
    let cmd = if recursive {
        ...
        format!("ls -la {} 2>/dev/null", quoted_path)
    };
    ...
    if recursive {
        ...
        for line in stdout.lines() {
            ...
            // Get file info
            let stat_cmd = format!(
                "stat -c '%n|%s|%Y|%W|%F' {} 2>/dev/null",
                quote_shell_arg(line)
            );
...
pub async fn file_exists(path: String) -> Result<serde_json::Value, String> {
    ...
    let file_type = lines.get(1).copied().unwrap_or("");
...
pub async fn write_file_binary(
    ...
    println!(
        "[Files] Writing binary file: {} ({} bytes base64)",
        path,
        content.len()
    );
    let _ = validate_path(&path)?;
