...
export async function getSession(sessionId: string): Promise<SessionMessagesResponse | null> {
  try {
    return await apiClient.invoke<SessionMessagesResponse>('get_session', { session_id: sessionId });
  } catch (error) {
...
export async function deleteSession(sessionId: string): Promise<ApiOkResponse> {
  return apiClient.invoke<ApiOkResponse>('delete_session', { session_id: sessionId });
}

export async function updateSessionTitle(sessionId: string, title: string): Promise<ApiOkResponse> {
  return apiClient.invoke<ApiOkResponse>('update_session_title', { session_id: sessionId, title });
}
